...
>
  <h3 className="text-[10px] font-medium text-gray-500 mb-3">{title}</h3>
  <p className={`mb-4 text-[15px] font-semibold leading-relaxed ${variant === 'gradient' ? 'text-white' : 'text-gray-900'}`}>
    {text}
  </p>
  ...
  {rating && (
    <div className="flex items-center gap-1 mb-4">
      ...
      <span className={`text-sm ml-2 ${variant === 'gradient' ? 'text-white' : 'text-gray-600'}`}>
        {rating.toFixed(1)}
      </span>
    </div>
  )}
  ...
  <div className="flex items-center mt-5 gap-3">
    ...
    />
    <div>
      <p className={`font-medium text-sm ${variant === 'gradient' ? 'text-white' : 'text-gray-900'}`}>
        {reviewer.name}
      </p>
  ...
  <img
    ...
    alt="Quote"
    className="absolute bottom-24 right-12 w-6 h-6" // Positioned at the bottom-right corner
  />
</div>
